...
///Cron 2 — Limpiar objetos FAILED en S3
...
//    private final StorageService storageService;
//
//    // Una vez por hora alcanza — no es urgente
//    @Scheduled(fixedDelay = 1, timeUnit = TimeUnit.HOURS)
//    @Transactional
//    public void deleteFailedObjects() {
//        // Solo archivos FAILED que tienen s3Key (los que llegaron a generar la key)
//        // y que llevan al menos 1 hora en FAILED para no interferir
//        // con confirms que puedan estar en vuelo
//        Instant cutoff = Instant.now().minus(1, ChronoUnit.HOURS);
//
//        List<File> failed = fileRepository.findFailedWithS3Key(cutoff);
//
//        for (File file : failed) {
//            try {
//                storageService.deleteObject(file.getS3Key());
//                file.setS3Key(null); // marcar que ya fue limpiado de S3
//            } catch (Exception e) {
//                // No propagamos — si falla hoy, lo intenta la próxima hora
//                // log del error con file.getId()
//            }
//        }
//
...
//""")
//List<File> findFailedWithS3Key(@Param("cutoff") Instant cutoff);
...
//    public void configurarLifecycle() {
//        s3Client.putBucketLifecycleConfiguration(r -> r
//                .bucket(bucket)
//                .lifecycleConfiguration(lc -> lc
//                        .rules(
//                                LifecycleRule.builder()
//                                        .id("delete-orphan-uploads")
//                                        .status(ExpirationStatus.ENABLED)
//                                        .filter(f -> f.prefix("workspaces/"))
